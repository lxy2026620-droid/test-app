      ...
      >
        ...
        <motion.div variants={fadeUp} className="glass-card rounded-2xl p-6 md:p-8">
          <div className="flex items-center gap-5">
            ...
            <button className="btn-ghost px-4 py-2 text-xs flex-shrink-0 hidden sm:inline-flex">
              编辑资料
            </button>
          </div>
        </motion.div>
        ...
        >
          {quickActions.map((action) => (
            <button
              key={action.label}
              className="glass-hover rounded-2xl p-4 flex items-center gap-3 text-sm font-medium text-dark-300 hover:text-white transition-colors"
            >
              <span className="w-9 h-9 rounded-xl bg-white/[0.04] flex items-center justify-center text-accent-400 flex-shrink-0">
                {action.icon}
              </span>
              {action.label}
            </button>
